...
;(function () {
    ...
    function setTabState(prefix, state) {
        const states = ["empty-state", "results", "loading"];
        states.forEach(s => {
            const el = $(`#${prefix}-${s.replace("-state", "-empty-state").replace("results", "results").replace("loading", "loading")}`);
        });
        // Simpler approach:
    ...
    $("#image-predict-btn").addEventListener("click", async () => {
        ...
        const model = $("#image-model-select").value;
    ...
    $("#text-predict-btn").addEventListener("click", async () => {
        ...
        if (!text) {
            showToast("Please enter text to classify.", "warning");
            return;
        }
    ...
    $("#multi-predict-btn").addEventListener("click", async () => {
        ...
        const model = $("#multi-model-select").value;
        const formData = new FormData();
        formData.append("file", file);
        formData.append("text", text);
        formData.append("model", model);
    ...
    document.addEventListener("keydown", (e) => {
        if (e.ctrlKey && e.key === "Enter") {
            ...
            if (!activeTab) return;
            const btn = activeTab.querySelector(".btn-primary");
            if (btn) btn.click();
        }
    });
